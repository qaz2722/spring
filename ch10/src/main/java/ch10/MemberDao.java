...
public class MemberDao {
	// singleton 하나의 객체로 DB에 CRUD에 처리
	...
	public int confirm(String id) {
		...
		try {
			...
			if (rs.next()) result = 1; // 해당하는 id있다
			else result = 0;  // 해당하는 id가 없다
		}catch (Exception e) {
	...
	public Member select(String id) {
		Member member = new Member();		PreparedStatement pstmt = null;
	...
	public int update(Member member) {
		...
		PreparedStatement pstmt = null;
		String sql = 
			"update member2 set password=?,name=?,address=?,tel=? where id=?";
		Connection conn = getConnection();
		try {
			pstmt = conn.prepareStatement(sql);			
